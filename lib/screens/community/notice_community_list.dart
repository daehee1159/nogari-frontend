...
class NoticeCommunityList extends StatefulWidget {
  const NoticeCommunityList({super.key});

  @override
  State<NoticeCommunityList> createState() => _NoticeCommunityListState();
}

class _NoticeCommunityListState extends State<NoticeCommunityList> {
  ...
  Widget build(BuildContext context) {
    ...
    return FutureBuilder(
      future: communityService.getNoticeCommunityList(communityProvider.getCurrentPage, communityProvider.getSize),
      builder: (BuildContext context, AsyncSnapshot snapshot) {
        if (snapshot.connectionState == ConnectionState.waiting) {
          ...
          communityProvider.setNoticeCommunityList = communityData.list;
                          ...
                          child: Column(
                            children: [
                              ...
                              ListView.builder(
                                ...
                                itemCount: communityProvider.getNoticeCommunityList.length,
                                    ...
                                    children: [
                                      Material(
                                        ...
                                        child: InkWell(
                                              ...
                                              children: [
                                                      ...
                                                      Expanded(
                                                        child: Container(
                                                          ...
                                                          child: Row(
                                                            children: [
                                                              Text(
                                                                communityProvider.getNoticeCommunityList[index].title,
                                                                style: Theme.of(context).textTheme.bodyMedium,
                                                              ),
                                                            ],
                                                          ),
                                                        ...
                                                        child: Padding(
                                                          ...
                                                          child: Row(
                                                            children: [
                                                              Expanded(
                                                                child: Text(
                                                                  'Lv.${communityProvider.getNoticeCommunityList[index].level} ${communityProvider.getNoticeCommunityList[index].nickname}',
                                                                  style: Theme.of(context).textTheme.bodySmall!.copyWith(fontSize: 11),
                                                                ),
                                                              ),
                                                              Expanded(
                                                                child: Text(
                                                                  communityProvider.getNoticeCommunityList[index].regDt.toString().substring(0, 10),
                                                                  style: Theme.of(context).textTheme.bodySmall!.copyWith(fontSize: 11),
                                                                ),
                                                              ),
                                                              Expanded(
                                                                child: Text(
                                                                  '조회 ${communityProvider.getNoticeCommunityList[index].viewCnt}',
                                                                  style: Theme.of(context).textTheme.bodySmall!.copyWith(fontSize: 11),
                                                                ),
                                                              ),
                                                              Expanded(
                                                                child: Text(
                                                                  '추천 ${communityProvider.getNoticeCommunityList[index].likeCnt}',
                                                                  style: Theme.of(context).textTheme.bodySmall!.copyWith(fontSize: 11),
                                                                ),
                                                              )
                                                            ],
                                                ...
                                                Expanded(
                                                  ...
                                                  child: Container(
                                                    ...
                                                    child: Center(
                                                      child: Text(
                                                        communityProvider.getCntOfComment[communityProvider.getNoticeCommunityList[index].boardSeq].toString(),
                                                        style: Theme.of(context).textTheme.bodySmall!.copyWith(fontSize: 11),
                                                      ),
                                                    ),
                                                  ),
                                          ...
                                          onTap: () async {
                                            // 이건 비동기 안해도 됨
                                            await CommunityService().addBoardViewCnt(communityProvider.getNoticeCommunityList[index].boardSeq);

                                            if (communityProvider.getNoticeCommunityList[index].fileName1 != null) {
                                              communityProvider.getNoticeCommunityList[index].fileUrl1 = await storage.downloadURL(communityProvider.getNoticeCommunityList[index].fileName1.toString(), 'community');
                                            }

                                            if (communityProvider.getNoticeCommunityList[index].fileName2 != null) {
                                              communityProvider.getNoticeCommunityList[index].fileUrl2 = await storage.downloadURL(communityProvider.getNoticeCommunityList[index].fileName2.toString(), 'community');
                                            }

                                            if (communityProvider.getNoticeCommunityList[index].fileName3 != null) {
                                              communityProvider.getNoticeCommunityList[index].fileUrl3 = await storage.downloadURL(communityProvider.getNoticeCommunityList[index].fileName3.toString(), 'community');
                                            }

                                            if (mounted) {
                                              Navigator.push(context, MaterialPageRoute(builder: (context) => CommunityDetailPage(community: communityProvider.getNoticeCommunityList[index])));
                                            }
                              ...
                              const CommonSearchBar(type: 'notice', isFirstTime: true,)
                            ],
                          ),
                        );
                      }
