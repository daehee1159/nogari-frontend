...
class LikeCommunityList extends StatefulWidget {
  const LikeCommunityList({super.key});

  @override
  State<LikeCommunityList> createState() => _LikeCommunityListState();
}

class _LikeCommunityListState extends State<LikeCommunityList> {
  ...
  Widget build(BuildContext context) {
    ...
    return FutureBuilder(
      future: communityService.getLikeCommunityList(communityProvider.getCurrentPage, communityProvider.getSize, communityProvider.getLikeCount),
      builder: (BuildContext context, AsyncSnapshot snapshot) {
        if (snapshot.connectionState == ConnectionState.waiting) {
          ...
          communityProvider.setLikeCommunityList = communityData.list;
                          ...
                          child: Column(
                            children: [
                              ...
                              ListView.builder(
                                ...
                                itemCount: communityProvider.getLikeCommunityList.length,
                                    ...
                                    children: [
                                      Material(
                                        ...
                                        child: InkWell(
                                              ...
                                              children: [
                                                      ...
                                                      Expanded(
                                                        child: Container(
                                                          ...
                                                          child: Row(
                                                            children: [
                                                              Text(
                                                                communityProvider.getLikeCommunityList[index].title,
                                                                style: Theme.of(context).textTheme.bodyMedium,
                                                              ),
                                                            ],
                                                          ),
                                                        ...
                                                        child: Padding(
                                                          ...
                                                          child: Row(
                                                            children: [
                                                              Expanded(
                                                                child: Text(
                                                                  'Lv.${communityProvider.getLikeCommunityList[index].level} ${communityProvider.getLikeCommunityList[index].nickname}',
                                                                  style: Theme.of(context).textTheme.bodySmall!.copyWith(fontSize: 11),
                                                                ),
                                                              ),
                                                              Expanded(
                                                                child: Text(
                                                                  communityProvider.getLikeCommunityList[index].regDt.toString().substring(0, 10),
                                                                  style: Theme.of(context).textTheme.bodySmall!.copyWith(fontSize: 11),
                                                                ),
                                                              ),
                                                              Expanded(
                                                                child: Text(
                                                                  '조회 ${communityProvider.getLikeCommunityList[index].viewCnt}',
                                                                  style: Theme.of(context).textTheme.bodySmall!.copyWith(fontSize: 11),
                                                                ),
                                                              ),
                                                              Expanded(
                                                                child: Text(
                                                                  '추천 ${communityProvider.getLikeCommunityList[index].likeCnt}',
                                                                  style: Theme.of(context).textTheme.bodySmall!.copyWith(fontSize: 11),
                                                                ),
                                                              )
                                                            ],
                                                ...
                                                Expanded(
                                                  ...
                                                  child: Container(
                                                    ...
                                                    child: Center(
                                                      child: Text(
                                                        communityProvider.getCntOfComment[communityProvider.getLikeCommunityList[index].boardSeq].toString(),
                                                        style: Theme.of(context).textTheme.bodySmall!.copyWith(fontSize: 11),
                                                      ),
                                                    ),
                                                  ),
                                          ...
                                          onTap: () async {
                                            // 이건 비동기 안해도 됨
                                            await CommunityService().addBoardViewCnt(communityProvider.getLikeCommunityList[index].boardSeq);

                                            if (communityProvider.getLikeCommunityList[index].fileName1 != null) {
                                              communityProvider.getLikeCommunityList[index].fileUrl1 = await storage.downloadURL(communityProvider.getLikeCommunityList[index].fileName1.toString(), 'community');
                                            }

                                            if (communityProvider.getLikeCommunityList[index].fileName2 != null) {
                                              communityProvider.getLikeCommunityList[index].fileUrl2 = await storage.downloadURL(communityProvider.getLikeCommunityList[index].fileName2.toString(), 'community');
                                            }

                                            if (communityProvider.getLikeCommunityList[index].fileName3 != null) {
                                              communityProvider.getLikeCommunityList[index].fileUrl3 = await storage.downloadURL(communityProvider.getLikeCommunityList[index].fileName3.toString(), 'community');
                                            }

                                            if (mounted) {
                                              Navigator.push(context, MaterialPageRoute(builder: (context) => CommunityDetailPage(community: communityProvider.getLikeCommunityList[index])));
                                            }
                                          },
                                        ),
                                      ),
                              ...
                              const CommonSearchBar(type: 'like', isFirstTime: true,)
                            ],
                          ),
                        );
                      }
