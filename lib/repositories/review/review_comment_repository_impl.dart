...
class ReviewCommentRepositoryImpl implements ReviewCommentRepository {
  final MemberRepository _memberRepository = MemberRepositoryImpl();

  @override
  Future<List<ReviewComment>> getCommentList(int boardSeq) async {
    ...
    List<ReviewComment> result =((json.decode(response.body) as List).map((e) => ReviewComment.fromJson(e)).toList());
    /// 차단회원 글 필터
    List<Block> blockMemberList = await _memberRepository.getBlockMember();
    List<ReviewComment> filteredList = [];
      ...
      for (int i = 0; i < result.length; i++) {
        ...
        for (int j = 0; j < blockMemberList.length; j++) {
          if (result[i].memberSeq == blockMemberList[j].blockMemberSeq) {
            ReviewComment commentDto = ReviewComment(
  ...
  @override
  Future<List<ReviewChildComment>> getChildCommentList(List<ReviewComment> list) async {
    List<ReviewChildComment> allList = [];
    ...
    for (int i = 0; i < list.length; i++) {
      ...
      List<ReviewChildComment> result =((json.decode(response.body) as List).map((e) => ReviewChildComment.fromJson(e)).toList());
