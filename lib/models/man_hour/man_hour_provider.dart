...
import 'package:nogari/models/man_hour/tax_info_dto.dart';

import 'man_hour_column_chart.dart';
import 'man_hour_dto.dart';

class ManHourProvider extends ChangeNotifier {
  List<ManHourDto> _events = [];
  List<ManHourDto> _searchList = [];
  ...
  int selectMonth = 0;
  List<ManHourDto> historyList = [];
  List<ManHourColumnChart> columnChartList = [];
  List<ManHourBarChart> barChartList = [];

  // 세금 정보
  TaxInfoDto taxInfo = TaxInfoDto();
  // 4대보험 가입 여부
  bool insuranceStatus = true;

  List<ManHourDto> get events => _events;
  List<ManHourDto> get getSearchList => _searchList;
  ...
  void addEvent(ManHourDto manHourDto) {
    _events.add(manHourDto);
    notifyListeners();
  }

  void addSearchList(ManHourDto manHourDto) {
    _searchList.add(manHourDto);
    notifyListeners();
  }
  ...
  void currentFetchData(List<ManHourDto> manHourDtoList) {
    ...
    for (int i = 0; i < manHourDtoList.length; i++) {
      totalMonthlyAmount = totalMonthlyAmount + manHourDtoList[i].totalAmount;
      totalMonthlyManHour = totalMonthlyManHour + manHourDtoList[i].manHour;
      avgMonthlyUnitPrice = avgMonthlyUnitPrice + manHourDtoList[i].unitPrice;
      totalMonthlyEtcPrice = totalMonthlyEtcPrice + manHourDtoList[i].etcPrice;
    }

    DateTime now = DateTime.parse(manHourDtoList[0].startDt);
  ...
  set setManHourList(List<ManHourDto> list) {
    _events = [];
    _events = list;
    // notifyListeners();
  }

  set setSearchList(List<ManHourDto> list) {
    _searchList = [];
    _searchList = list;
    // notifyListeners();
  }
  ...
  set setHistoryList(List<ManHourDto> list) {
    historyList = [];
    historyList = list;
    // notifyListeners();
  }
  ...
  set setTaxInfo(TaxInfoDto taxInfoDto) {
    taxInfo = taxInfoDto;
    notifyListeners();
  }
