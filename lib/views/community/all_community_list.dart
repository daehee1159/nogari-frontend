...
import 'package:nogari/views/community/information_use.dart';
...
class _AllCommunityListState extends State<AllCommunityList> {
  ...
  Widget build(BuildContext context) {
    ...
    return FutureBuilder(
        future: _communityRepository.getCommunityList(communityViewModel.getCurrentPage, communityViewModel.getSize),
                            ...
                            child: Column(
                              children: [
                                ...
                                ListView.builder(
                                  ...
                                  itemCount: viewModel.getCommunityList.length,
                                      ...
                                      children: [
                                        Material(
                                          ...
                                          child: InkWell(
                                                ...
                                                children: [
                                                        ...
                                                        Expanded(
                                                          child: Container(
                                                            ...
                                                            child: Row(
                                                              children: [
                                                                Text(
                                                                  viewModel.getCommunityList[index].title,
                                                                  style: Theme.of(context).textTheme.bodyMedium,
                                                                ),
                                                              ],
                                                            ),
                                                          ...
                                                          child: Padding(
                                                            ...
                                                            child: Row(
                                                              children: [
                                                                Expanded(
                                                                  child: Text(
                                                                    'Lv.${viewModel.getCommunityList[index].level} ${viewModel.getCommunityList[index].nickname}',
                                                                    style: Theme.of(context).textTheme.bodySmall!.copyWith(fontSize: 11),
                                                                  ),
                                                                ),
                                                                Expanded(
                                                                  child: Text(
                                                                    viewModel.getCommunityList[index].regDt.toString().substring(0, 10),
                                                                    style: Theme.of(context).textTheme.bodySmall!.copyWith(fontSize: 11),
                                                                  ),
                                                                ),
                                                                Expanded(
                                                                  child: Text(
                                                                    '조회 ${viewModel.getCommunityList[index].viewCnt}',
                                                                    style: Theme.of(context).textTheme.bodySmall!.copyWith(fontSize: 11),
                                                                  ),
                                                                ),
                                                                Expanded(
                                                                  child: Text(
                                                                    '추천 ${viewModel.getCommunityList[index].likeCnt}',
                                                                    style: Theme.of(context).textTheme.bodySmall!.copyWith(fontSize: 11),
                                                                  ),
                                                                )
                                                              ],
                                                  ...
                                                  Expanded(
                                                    ...
                                                    child: Container(
                                                      ...
                                                      child: Center(
                                                        child: Text(
                                                          viewModel.getCntOfComment[viewModel.getCommunityList[index].boardSeq].toString(),
                                                          style: Theme.of(context).textTheme.bodySmall!.copyWith(fontSize: 11),
                                                        ),
                                                      ),
                                                    ),
                                            ...
                                            onTap: () async {
                                              // 이건 비동기 안해도 됨
                                              await _communityRepository.addBoardViewCnt(viewModel.getCommunityList[index].boardSeq);

                                              if (viewModel.getCommunityList[index].fileName1 != null) {
                                                viewModel.getCommunityList[index].fileUrl1 = await storage.downloadURL(viewModel.getCommunityList[index].fileName1.toString(), 'community');
                                              }

                                              if (viewModel.getCommunityList[index].fileName2 != null) {
                                                viewModel.getCommunityList[index].fileUrl2 = await storage.downloadURL(viewModel.getCommunityList[index].fileName2.toString(), 'community');
                                              }

                                              if (viewModel.getCommunityList[index].fileName3 != null) {
                                                viewModel.getCommunityList[index].fileUrl3 = await storage.downloadURL(viewModel.getCommunityList[index].fileName3.toString(), 'community');
                                              }

                                              List<CommunityLike> fetchData = await _communityRepository.getBoardLikeCnt(viewModel.getCommunityList[index].boardSeq);

                                              for (int i = 0; i < fetchData.length; i++) {
                                                if (fetchData[i].memberSeq == memberViewModel.getMemberSeq) {
                                                  viewModel.getCommunityList[index].isMyPress = true;
                                                }
                                              }

                                              if (mounted) {
                                                Navigator.push(context, MaterialPageRoute(builder: (context) => CommunityDetailPage(community: viewModel.getCommunityList[index])));
                                              }
                                            },
                                          ),
                                        ),
