...
class _NoticeCommunityListState extends State<NoticeCommunityList> {
  ...
  Widget build(BuildContext context) {
    ...
    return FutureBuilder(
      future: _communityRepository.getNoticeCommunityList(communityViewModel.getCurrentPage, communityViewModel.getSize),
      builder: (BuildContext context, AsyncSnapshot snapshot) {
        if (snapshot.connectionState == ConnectionState.waiting) {
          ...
          communityViewModel.setNoticeCommunityList = communityData.list;
                          ...
                          child: Column(
                            children: [
                              ...
                              ListView.builder(
                                ...
                                itemCount: viewModel.getNoticeCommunityList.length,
                                    ...
                                    children: [
                                      Material(
                                        ...
                                        child: InkWell(
                                              ...
                                              children: [
                                                      ...
                                                      Expanded(
                                                        child: Container(
                                                          ...
                                                          child: Row(
                                                            children: [
                                                              Text(
                                                                viewModel.getNoticeCommunityList[index].title,
                                                                style: Theme.of(context).textTheme.bodyMedium,
                                                              ),
                                                            ],
                                                          ),
                                                        ...
                                                        child: Padding(
                                                          ...
                                                          child: Row(
                                                            children: [
                                                              Expanded(
                                                                child: Text(
                                                                  'Lv.${viewModel.getNoticeCommunityList[index].level} ${viewModel.getNoticeCommunityList[index].nickname}',
                                                                  style: Theme.of(context).textTheme.bodySmall!.copyWith(fontSize: 11),
                                                                ),
                                                              ),
                                                              Expanded(
                                                                child: Text(
                                                                  viewModel.getNoticeCommunityList[index].regDt.toString().substring(0, 10),
                                                                  style: Theme.of(context).textTheme.bodySmall!.copyWith(fontSize: 11),
                                                                ),
                                                              ),
                                                              Expanded(
                                                                child: Text(
                                                                  '조회 ${viewModel.getNoticeCommunityList[index].viewCnt}',
                                                                  style: Theme.of(context).textTheme.bodySmall!.copyWith(fontSize: 11),
                                                                ),
                                                              ),
                                                              Expanded(
                                                                child: Text(
                                                                  '추천 ${viewModel.getNoticeCommunityList[index].likeCnt}',
                                                                  style: Theme.of(context).textTheme.bodySmall!.copyWith(fontSize: 11),
                                                                ),
                                                              )
                                                            ],
                                                ...
                                                Expanded(
                                                  ...
                                                  child: Container(
                                                    ...
                                                    child: Center(
                                                      child: Text(
                                                        viewModel.getCntOfComment[viewModel.getNoticeCommunityList[index].boardSeq].toString(),
                                                        style: Theme.of(context).textTheme.bodySmall!.copyWith(fontSize: 11),
                                                      ),
                                                    ),
                                                  ),
                                          ...
                                          onTap: () async {
                                            // 이건 비동기 안해도 됨
                                            await _communityRepository.addBoardViewCnt(viewModel.getNoticeCommunityList[index].boardSeq);

                                            if (viewModel.getNoticeCommunityList[index].fileName1 != null) {
                                              viewModel.getNoticeCommunityList[index].fileUrl1 = await _storage.downloadURL(viewModel.getNoticeCommunityList[index].fileName1.toString(), 'community');
                                            }

                                            if (viewModel.getNoticeCommunityList[index].fileName2 != null) {
                                              viewModel.getNoticeCommunityList[index].fileUrl2 = await _storage.downloadURL(viewModel.getNoticeCommunityList[index].fileName2.toString(), 'community');
                                            }

                                            if (viewModel.getNoticeCommunityList[index].fileName3 != null) {
                                              viewModel.getNoticeCommunityList[index].fileUrl3 = await _storage.downloadURL(viewModel.getNoticeCommunityList[index].fileName3.toString(), 'community');
                                            }

                                            if (mounted) {
                                              Navigator.push(context, MaterialPageRoute(builder: (context) => CommunityDetailPage(community: viewModel.getNoticeCommunityList[index])));
                                            }
