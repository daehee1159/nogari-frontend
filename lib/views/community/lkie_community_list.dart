...
class _LikeCommunityListState extends State<LikeCommunityList> {
  ...
  Widget build(BuildContext context) {
    ...
    return FutureBuilder(
      future: _communityRepository.getLikeCommunityList(communityViewModel.getCurrentPage, communityViewModel.getSize, communityViewModel.getLikeCount),
      builder: (BuildContext context, AsyncSnapshot snapshot) {
        if (snapshot.connectionState == ConnectionState.waiting) {
          ...
          communityViewModel.setLikeCommunityList = communityData.list;
                          ...
                          child: Column(
                            children: [
                              ...
                              ListView.builder(
                                ...
                                itemCount: viewModel.getLikeCommunityList.length,
                                    ...
                                    children: [
                                      Material(
                                        ...
                                        child: InkWell(
                                              ...
                                              children: [
                                                      ...
                                                      Expanded(
                                                        child: Container(
                                                          ...
                                                          child: Row(
                                                            children: [
                                                              Text(
                                                                viewModel.getLikeCommunityList[index].title,
                                                                style: Theme.of(context).textTheme.bodyMedium,
                                                              ),
                                                            ],
                                                          ),
                                                        ...
                                                        child: Padding(
                                                          ...
                                                          child: Row(
                                                            children: [
                                                              Expanded(
                                                                child: Text(
                                                                  'Lv.${viewModel.getLikeCommunityList[index].level} ${viewModel.getLikeCommunityList[index].nickname}',
                                                                  style: Theme.of(context).textTheme.bodySmall!.copyWith(fontSize: 11),
                                                                ),
                                                              ),
                                                              Expanded(
                                                                child: Text(
                                                                  viewModel.getLikeCommunityList[index].regDt.toString().substring(0, 10),
                                                                  style: Theme.of(context).textTheme.bodySmall!.copyWith(fontSize: 11),
                                                                ),
                                                              ),
                                                              Expanded(
                                                                child: Text(
                                                                  '조회 ${viewModel.getLikeCommunityList[index].viewCnt}',
                                                                  style: Theme.of(context).textTheme.bodySmall!.copyWith(fontSize: 11),
                                                                ),
                                                              ),
                                                              Expanded(
                                                                child: Text(
                                                                  '추천 ${viewModel.getLikeCommunityList[index].likeCnt}',
                                                                  style: Theme.of(context).textTheme.bodySmall!.copyWith(fontSize: 11),
                                                                ),
                                                              )
                                                            ],
                                                ...
                                                Expanded(
                                                  ...
                                                  child: Container(
                                                    ...
                                                    child: Center(
                                                      child: Text(
                                                        viewModel.getCntOfComment[viewModel.getLikeCommunityList[index].boardSeq].toString(),
                                                        style: Theme.of(context).textTheme.bodySmall!.copyWith(fontSize: 11),
                                                      ),
                                                    ),
                                                  ),
                                          ...
                                          onTap: () async {
                                            // 이건 비동기 안해도 됨
                                            await _communityRepository.addBoardViewCnt(viewModel.getLikeCommunityList[index].boardSeq);

                                            if (viewModel.getLikeCommunityList[index].fileName1 != null) {
                                              viewModel.getLikeCommunityList[index].fileUrl1 = await storage.downloadURL(viewModel.getLikeCommunityList[index].fileName1.toString(), 'community');
                                            }

                                            if (viewModel.getLikeCommunityList[index].fileName2 != null) {
                                              viewModel.getLikeCommunityList[index].fileUrl2 = await storage.downloadURL(viewModel.getLikeCommunityList[index].fileName2.toString(), 'community');
                                            }

                                            if (viewModel.getLikeCommunityList[index].fileName3 != null) {
                                              viewModel.getLikeCommunityList[index].fileUrl3 = await storage.downloadURL(viewModel.getLikeCommunityList[index].fileName3.toString(), 'community');
                                            }

                                            if (mounted) {
                                              Navigator.push(context, MaterialPageRoute(builder: (context) => CommunityDetailPage(community: viewModel.getLikeCommunityList[index])));
                                            }
                                          },
                                        ),
                                      ),
